...
myTrace = do
    ...
    case m of
        ...
        Just ts -> do
           ...
           -- Extras.logInfo $ "Close Token Sale " ++ show ts
            void $ Emulator.waitNSlots 5

            -- get an error now
            
            Extras.logInfo $ "Calling add tokens to the closed token sale " ++ show ts
            callEndpoint @"add tokens" h1 50_000
