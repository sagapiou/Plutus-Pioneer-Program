...
import           Week08.HWTokenSale                   (TokenSale (..), TSStartSchema, TSUseSchema, startEndpoint, useEndpoints)
...
instance ContractModel TSModel where
    ...
    arbitraryAction _ = oneof $
        ...
        [ CloseTokenSale <$> genWallet <*> genWallet ]
    ...
    nextState (CloseTokenSale v w) = do
        when (v == w) $ do
            m <- getTSState v
            case m of
                Just t -> do
    ...
    perform h _ cmd = case cmd of
        ...
        (CloseTokenSale v w) -> callEndpoint @"close"    (h $ UseKey v w) ()                                                   >> delay 1
    ...
    precondition s (CloseTokenSale v _) = isJust    $ getTSState' s v
