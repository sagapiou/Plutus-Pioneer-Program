...
transition ts s r = case (stateValue s,  stateData s, r) of
    (v, p, SetPrice n)   | isJust p && n >= 0           -> Just ( Constraints.mustBeSignedBy (tsSeller ts)
                                                                , State (Just n) v
                                                                )
    (v, p, AddTokens n)  | isJust p && n > 0            -> Just ( mempty
                                                                , State p $
                                                                  v                                       <>
                                                                  assetClassValue (tsToken ts) n
                                                                )
    (v, p, BuyTokens n)  | isJust p && n > 0            -> Just ( mempty
                                                                , State p $
                                                                  v                                       <>
                                                                  assetClassValue (tsToken ts) (negate n) <>
                                                                  lovelaceValueOf (n * fromJust(p))
                                                                )
    (v, p, Withdraw n l) | isJust p && n >= 0 && l >= 0 -> Just ( Constraints.mustBeSignedBy (tsSeller ts)
                                                                , State p $
                                                                  v                                       <>
                                                                  assetClassValue (tsToken ts) (negate n) <>
...
useEndpoints ts = forever
  ...
  where
    ...
    close'     = endpoint @"close"             $ const $ closeTS ts 
