...
mkPolicy pkh deadline () ctx = traceIfFalse "Not Signed By the correct wallet" signedCorrectly &&
                          traceIfFalse "Deadline has passed" deadlinePassed
    where
        info :: TxInfo
        info = scriptContextTxInfo ctx

        signedCorrectly :: Bool
        signedCorrectly = txSignedBy (scriptContextTxInfo ctx) pkh

        deadlinePassed :: Bool
        deadlinePassed = contains (to deadline) $ txInfoValidRange info
